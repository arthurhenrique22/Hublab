      ...
      <Container>
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={7}>
            ...
            <Grid container spacing={2} sx={{ ml: { xs: 0, md: 1 } }}>
              {data.map(({ description, icon }, index) => (
                <Grid key={String(index)} item xs={12} md={8}>
                  <Box sx={{ px: 1, py: 3, boxShadow: 1, borderRadius: 4, display: 'flex', alignItems: 'center' }}>
                    <Box
                      sx={{
                        mr: 2,
                        ...
                        '& svg': {
                          fontSize: 24, // Tamanho ajustado do ícone
                        },
                      }}
                    >
                      {icon}
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, alignItems: 'flex-start' }}>
                      <Typography sx={{ lineHeight: 1.5, color: 'text.secondary' }} variant="subtitle1">
                        {description}
                      </Typography>
                    </Box>
                  </Box>
